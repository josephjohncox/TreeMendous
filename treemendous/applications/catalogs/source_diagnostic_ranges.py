...
class SourceDiagnosticCatalog:
    ...
    def update(
        ...
        *,
        ...
        with self._lock:
            ...
            parsed = (
                current.payload.severity if severity is None else Severity(severity)
            )
            payload = replace(
                current.payload,
                version=(
                    current.payload.version if version is None else _version(version)
                ),
                severity=parsed,
                message=(
    ...
    def remap_edit(
        ...
        *,
        ...
        Insertions use explicit half-open affinity: a start exactly at the
        insertion attaches to following text and moves right, while an end
        exactly there attaches to preceding text and stays left. A diagnostic
        enclosing the insertion therefore grows to cover the inserted text.

        Endpoints inside replaced text retain their displacement from the edit
        start, clamped to the replacement length. Diagnostics collapsed by an
        edit are removed. All matching diagnostics are updated or removed in
        one catalog transaction; the returned identities preserve insertion
        order.
        """
        validate_coordinate(edit_start, "edit_start")
        validate_coordinate(edit_end, "edit_end")
        validate_coordinate(replacement_length, "replacement_length")
        if edit_start < 0 or edit_end < edit_start:
            raise ValueError("edit range must be nonnegative and ordered")
        ...
        with self._lock:
            ...
            updates: list[tuple[DiagnosticHandle, str, int, int, Diagnostic]] = []
            removals: list[tuple[DiagnosticHandle, str]] = []
            for record in self._index.snapshot().records:
                ...
                if diagnostic.file != file or diagnostic.version != old_version:
                    continue
                mapped_start = map_start(record.start)
                mapped_end = map_end(record.end)
                changed.append(record.handle)
                if mapped_start >= mapped_end:
                    removals.append((record.handle, record.handle.owner))
                else:
                    updates.append(
                        (
                            record.handle,
                            ...
                            mapped_start,
                            mapped_end,
                            replace(diagnostic, version=target_version),
                        )
                    )
            self._index.replace_batch(updates=updates, removals=removals)
