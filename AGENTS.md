...
- `treemendous/`: core package.
  - `treemendous/basic/`: internal pure-Python geometry engines.
  - `treemendous/cpp/`: pybind11 extensions; `gpu/` and `metal/` for platform builds.
  - `treemendous/backends/registry.py`: canonical backend discovery and construction.
...
- `uv sync` installs base deps; `uv sync --all-extras` enables build, quality, and test tooling.
...
- Stable backend checks live in `tests/unit/test_stable_backends.py`; run `just test` before PRs.
