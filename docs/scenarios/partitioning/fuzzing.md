...
`input_for()` exposes deterministic generation, `execute_claim()` runs a band, and `run(..., fail_first_claim=True)` can exercise retry behavior without pretending target crashes are worker loss. Snapshots include executed ordinals, deduplicated crashes, and retry count; `audit_snapshot()` adds claim/event evidence but is not restorable because the target and generation configuration are omitted. Counts, sizes, seed, target, and ordinal boundaries are validated.
