...
Use `parameters_for(id)` to inspect the ID mapping, `evaluate_claim()` for explicit workers, or `run()` to evaluate the grid. `ranking()` and `snapshot()` expose results without insertion-order dependence; `checkpoint()` also captures private claims/events. Empty spaces/axes, invalid names, non-sequence axes, invalid IDs, non-callable objectives, and non-finite scores fail explicitly.
