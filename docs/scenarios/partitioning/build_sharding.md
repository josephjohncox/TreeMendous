...
Claims cover contiguous shard IDs. `execute_claim()` checks external dependencies before recording tasks and abandons a claim on ordering failure. `run()` executes in shard order. `shards`, `snapshot()`, and `audit_snapshot()` expose weights, topological order, completion, and local claim/event evidence. The audit snapshot is not restorable because it omits the source DAG.
