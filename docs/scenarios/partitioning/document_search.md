...
Use `claim()` plus `search_claim()` when driving workers explicitly, or `run(shard_size=...)` for local execution. `snapshot()` exposes the normalized query, merged hits, and claimed-document count; `audit_snapshot()` pairs that evidence with claim/event state but is not restorable because it omits source documents and index state. Invalid IDs, non-string text, empty inputs, and tokenless queries fail before claims are created.
