...
`apply_claim()` handles explicit offset windows and `run()` drains them locally. `audit_snapshot()` contains sorted state, applied offsets, and private claim/event evidence, but it is not restorable because it omits the source events. Duplicate/negative offsets, invalid operations, missing values, wrong value types, empty event sets, and increments of strings are rejected or rolled back explicitly.
