...
`merge_claim()` produces complete term outputs, `run(band_size=...)` drains the job, and snapshots expose source term order and merged postings. `audit_snapshot()` adds private coordination evidence but is not restorable because source segments are omitted. Empty segments/term universes, invalid keys, malformed/unsorted postings, and invalid bands fail explicitly.
