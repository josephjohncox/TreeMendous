...
`expand(width=...)` processes a bounded frontier band and `run()` continues until no reachable vertices remain. `snapshot()` reports expansion order, visited distances, and the pending frontier; `audit_snapshot()` adds local claim/event evidence but is not restorable because it omits the graph, queued set, and start identity. Dangling edges, invalid names, empty graphs, invalid starts, and non-sequence adjacency fail before execution. Disconnected vertices remain unvisited by correct BFS semantics.
