...
`evaluate_claim()` executes one band and `run(shard_size=...)` exhausts prefixes. Results contain both ordinal and a variable-order Boolean tuple. `snapshot()` captures configuration and ordered solutions; `audit_snapshot()` includes claim/event evidence but is not restorable because the clauses are omitted. Empty CNFs/clauses, literal zero, out-of-range literals, invalid variable counts, and invalid prefix widths are rejected.
