...
def test_basic_functionality():
    ...
    for impl_name, manager in implementations:
        ...
        print("    ✓ Empty manager works correctly")
        ...
        print("    ✓ Single interval works correctly")
        ...
        print("    ✓ Advanced queries work correctly")
...
def test_summary_analytics():
    ...
    for impl_name, manager in implementations:
        ...
        manager.reserve_interval(
            500, 600
        )  # Creates [0,100), [200,300), [400,500), [600,1000)
        ...
        print(
            f"    ✓ Fragmentation: {summary.fragmentation_index:.2f} (expected ≈ {expected_fragmentation:.2f})"
        )
        print(
            f"    ✓ Total free: {summary.total_free_length} (expected {expected_total})"
        )
        print(
            f"    ✓ Largest: {summary.largest_interval_length} (expected {expected_largest})"
        )
...
def test_best_fit_queries():
    ...
    for impl_name, manager in implementations:
        ...
        assert largest.length == expected_size, (
            f"{impl_name}: Largest should be {expected_size}, got {largest.length}"
        )

        print(
            f"    ✓ Largest available: [{largest.start}, {largest.end}), size={largest.length}"
        )


def test_mixed_operation_summary_consistency():
    """Mixed operations leave repeated summaries consistent with geometry."""
    implementations = []
    if PY_AVAILABLE:
        implementations.append(("Python", BoundarySummaryManager()))
    if CPP_AVAILABLE:
    ...
    for impl_name, manager in implementations:
        ...
        first = normalized(manager.get_availability_stats())
        second = normalized(manager.get_availability_stats())
        assert first == second, f"{impl_name}: repeated summary changed"
        assert first[0] == manager.get_total_available_length()
...
def main():
    ...
    tests = [
        ...
        test_mixed_operation_summary_consistency,
    ...
    if passed == len(tests):
        ...
        print("   • Boundary-based efficiency with summary analytics")
        print("   • Cached summary statistics")
        print("   • Best-fit and largest-available queries")
        print("   • Python/C++ implementation equivalence")
