...
def test_restore_rejects_forged_tracker_origin_atomically() -> None:
    ring = RingBuffer(4, sequence_modulus=8, initial_sequence=6)
    ring.produce(2, epoch_hint=0)
    checkpoint = ring.checkpoint()
    forged_sequences = replace(checkpoint.sequences, origin=0)
    before = ring.snapshot()

    with pytest.raises(ValueError, match="origin"):
        ring.restore(replace(checkpoint, sequences=forged_sequences))

    assert ring.snapshot() == before


def test_restore_rejects_impossible_overwrite_total_atomically() -> None:
    ring = RingBuffer(4, full_policy=FullPolicy.OVERWRITE)
    checkpoint = ring.checkpoint()
    before = ring.snapshot()

    with pytest.raises(ValueError, match="ring cursors"):
        ring.restore(replace(checkpoint, overwritten=999))
