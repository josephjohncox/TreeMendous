...
def test_weighted_shards_are_contiguous_and_dependency_safe() -> None:
    tasks = (
        BuildTask("a", (), 5), BuildTask("b", (), 1),
        BuildTask("c", ("a",), 2), BuildTask("d", ("b", "c"), 3),
    )
    ...
    assert tuple(name for shard in engine.shards for name in shard.tasks) == engine.snapshot().topological_order
...
def test_build_sharding_rejects_cycles_and_missing_dependencies() -> None:
    with pytest.raises(ValueError, match="cycle"):
        BuildShardingEngine((BuildTask("a", ("b",)), BuildTask("b", ("a",))), shard_count=1)
