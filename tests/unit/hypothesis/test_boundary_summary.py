...
from treemendous.basic.boundary import IntervalManager as RegularBoundary
from treemendous.basic.boundary_summary import (
    BoundarySummaryManager as PyBoundarySummary,
)
...
def validate_boundary_summary_invariants(manager: Any) -> None:
    ...
    assert summary.largest_interval_length >= 0, (
        "Largest interval length cannot be negative"
    )

    # Consistency checks
    if summary.interval_count > 0:
        assert summary.total_free_length > 0, (
            "If intervals exist, total free should be positive"
        )
        assert summary.largest_interval_length > 0, (
            "If intervals exist, largest should be positive"
        )
        assert summary.avg_interval_length > 0, (
            "If intervals exist, average should be positive"
        )

    # Bounds checks
    assert 0.0 <= summary.fragmentation_index <= 1.0, (
        f"Fragmentation should be [0,1], got {summary.fragmentation_index}"
    )
    assert 0.0 <= summary.utilization <= 1.0, (
        f"Utilization should be [0,1], got {summary.utilization}"
    )
    ...
    if hasattr(manager, "get_intervals"):
        ...
        assert summary.interval_count == actual_count, (
            f"Count mismatch: summary {summary.interval_count} vs actual {actual_count}"
        )
        assert summary.total_free_length == actual_total, (
            f"Length mismatch: summary {summary.total_free_length} vs actual {actual_total}"
        )

        if actual_intervals:
            actual_largest = max(end - start for start, end in actual_intervals)
            assert summary.largest_interval_length == actual_largest, (
                f"Largest mismatch: summary {summary.largest_interval_length} vs actual {actual_largest}"
            )
...
def test_py_boundary_summary_operations(operations: List[Tuple[int, int]]) -> None:
    ...
    if stats["largest_chunk"] > 0:
        ...
        assert largest.length == stats["largest_chunk"], (
            "Largest interval size should match summary"
        )
...
def test_py_boundary_summary_mixed_operations(
...
def test_py_cpp_boundary_summary_equivalence():
    ...
    assert py_stats["total_free"] == cpp_stats.total_free, (
        f"Total free mismatch: Python {py_stats['total_free']} vs C++ {cpp_stats.total_free}"
    )
    assert py_stats["free_chunks"] == cpp_stats.free_chunks, (
        f"Chunk count mismatch: Python {py_stats['free_chunks']} vs C++ {cpp_stats.free_chunks}"
    )
    assert py_stats["largest_chunk"] == cpp_stats.largest_chunk, (
        f"Largest chunk mismatch: Python {py_stats['largest_chunk']} vs C++ {cpp_stats.largest_chunk}"
    )
...
def test_boundary_summary_caching():
    ...
    assert perf2.cache_hits > perf1.cache_hits, (
        "Cache hits should increase on repeated access"
    )
    ...
    assert summary3.total_free_length != summary1.total_free_length, (
        "Summary should update after modification"
    )
...
def test_boundary_summary_advanced_queries():
    ...
    for impl_name, manager in implementations:
        ...
        assert best_fit_50.length == 50, (
            f"{impl_name}: Best fit should be exactly 50 units"
        )
        ...
        assert largest.length == expected_largest, (
            f"{impl_name}: Largest should be {expected_largest}, got {largest.length}"
        )
        ...
        assert total_free == expected_free, (
            f"{impl_name}: Total free should be {expected_free}, got {total_free}"
        )
...
def test_boundary_summary_performance():
    ...
    if not implementations:
        print(
            "⚠️  No boundary summary implementations available for performance testing"
        )
        return
    ...
    for impl_name, manager in implementations:
        ...
        if hasattr(manager, "get_availability_stats"):
            ...
            print(
                f"  Final state: {free_chunks} chunks, {fragmentation:.1%} fragmentation"
            )
...
def test_boundary_summary_vs_regular_boundary():
    ...
    assert regular_total == summary_total, (
        f"Total available mismatch: regular {regular_total} vs summary {summary_total}"
    )
    ...
    assert "fragmentation" in stats, (
        "Summary manager should provide fragmentation analysis"
    )
