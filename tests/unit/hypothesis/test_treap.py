...
def validate_treap_invariants(treap: IntervalTreap) -> None:
    ...
    assert intervals == sorted_intervals, (
        "BST property violated: intervals not in sorted order"
    )
    ...
    def check_node_stats(node):
        ...
        expected_height = 1 + max(
            TreapNode.get_height(node.left), TreapNode.get_height(node.right)
        )
        assert node.height == expected_height, (
            f"Height inconsistency: {node.height} vs {expected_height}"
        )

        # Size consistency
        expected_size = (
            1 + TreapNode.get_size(node.left) + TreapNode.get_size(node.right)
        )
        assert node.subtree_size == expected_size, (
            f"Size inconsistency: {node.subtree_size} vs {expected_size}"
        )
        ...
        assert node.total_length == expected_total, (
            f"Total length inconsistency: {node.total_length} vs {expected_total}"
        )
...
def test_treap_find_operations(length: int) -> None:
    ...
    if result:
        ...
        assert found_available, (
            f"Interval [{result.start}, {result.end}] not actually available"
        )
    else:
...
def test_treap_random_sampling():
    ...
    assert sampled_intervals.issubset(available_intervals), (
        "Sampled intervals not in available set"
    )
...
def test_treap_split_merge():
    ...
    assert (
        left_treap.get_total_available_length()
        + right_treap.get_total_available_length()
        == original_length
    )
    ...
    for interval in left_intervals:
        assert interval.start < 45, (
            f"Left treap should only contain intervals starting before 45, found [{interval.start}, {interval.end})"
        )

    for interval in right_intervals:
        assert interval.start >= 45, (
            f"Right treap should only contain intervals starting at/after 45, found [{interval.start}, {interval.end})"
        )
...
def test_treap_probabilistic_balance():
    ...
    assert stats["height"] < 2 * expected_height, (
        f"Height {stats['height']} too large for size {n}"
    )
...
def test_treap_interval_merging(intervals: List[Tuple[int, int]]) -> None:
    ...
    for i in range(len(result_intervals) - 1):
        ...
        assert curr_interval.end <= next_interval.start, (
            f"Overlapping intervals: [{curr_interval.start}, {curr_interval.end}) and [{next_interval.start}, {next_interval.end})"
        )
...
def test_treap_performance_characteristics():
    ...
    print(
        f"  Insertion time: {insertion_time:.3f}s ({insertion_time / len(intervals) * 1000:.2f}ms per operation)"
    )
    print(
        f"  Query time: {query_time:.3f}s ({query_time / 100 * 1000:.2f}ms per query)"
    )
    print(
        f"  Tree height: {stats['height']} (expected: {stats['expected_height']:.1f})"
    )
...
def test_treap_rank_operations():
    ...
    for i, (start, end) in enumerate(test_intervals):
        ...
        assert rank == i, (
            f"Rank mismatch for [{start}, {end}): expected {i}, got {rank}"
        )
    ...
    for i in range(len(test_intervals)):
        sample = treap.sample_random_interval()
        if sample:
            assert (sample.start, sample.end) in [
                (interval.start, interval.end) for interval in treap.get_intervals()
            ], "Selected interval not in treap"
...
def test_treap_correctness_vs_simple_tree(intervals: List[Tuple[int, int]]) -> None:
    ...
    assert (
        treap.get_total_available_length() == simple_tree.get_total_available_length()
    )
...
def test_treap_stress_operations():
    ...
    if stats["size"] > 0:
        assert stats["balance_factor"] < 5.0, (
            f"Tree too unbalanced after stress test: {stats['balance_factor']}"
        )
...
def test_treap_probabilistic_properties():
    ...
    print(
        f"  Average balance factor: {sum(balance_factors) / len(balance_factors):.2f}"
    )
    ...
    assert avg_height < 2.5 * expected_height, (
        f"Average height too large: {avg_height} vs expected {expected_height}"
    )
    assert max(balance_factors) < 3.5, (
        f"Some trees too unbalanced: max factor {max(balance_factors)}"
    )
