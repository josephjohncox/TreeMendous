...
def test_reserve_empty_interval(start: int, end: int) -> None:
    ...
    with pytest.raises(ValueError):
        manager.reserve_interval(start, start)
    with pytest.raises(ValueError):
        manager.reserve_interval(end, end)
...
@given(
    ...
    ).map(lambda ops: [(op, start, end) for op, start, end in ops])
)
...
@given(
    st.integers(min_value=0, max_value=999), st.integers(min_value=1, max_value=1000)
)
