...
def validate_tree_invariants(tree: SummaryIntervalTree) -> None:
    ...
    def check_node_invariants(node):
        ...
        expected_height = 1 + max(
            node.get_height(node.left), node.get_height(node.right)
        )
...
def test_mixed_operations_invariants(operations: List[Tuple[str, int, int]]) -> None:
    ...
    for op, start, end in operations:
        ...
        if (
            summary.earliest_free_start is not None
            and summary.latest_free_end is not None
        ):
...
def test_fragmentation_metrics() -> None:
    ...
    expected_fragmentation = 1.0 - (
        expected_largest / expected_total_free
    )  # 1 - 200/600 = 2/3
...
@given(
    st.integers(min_value=28800, max_value=86400),
