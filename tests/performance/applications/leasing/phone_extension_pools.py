"""Correctness-attested benchmark for phone extension leasing."""
...
def run_benchmark(
    ...
    """Run a bounded deterministic extension lifecycle and attest evidence."""
    ...
    engine = PhoneExtensionPool(
        ...
        last_extension=first_extension + capacity - 1,
        emergency_numbers=(),
        clock=clock,
    )
    adapter = LeasingBenchmarkAdapter(
        scenario_id="phone-extension-pools",
        primary_scope=_SCOPE,
        scopes=(_SCOPE,),
        domains={_SCOPE: ((first_extension, first_extension + capacity),)},
